...
export function AppShell({ children, activeNavigation, onNavigate }: AppShellProps) {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#F8FAFC] text-[#111827] lg:flex">

      {/* ── Mobile header ── */}
      <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-[#EEF2F7] bg-white px-4 lg:hidden"
        style={{ boxShadow: '0 1px 0 #EEF2F7' }}>
        <Brand />
        <div className="flex items-center gap-1.5">
          ...
          <button type="button" onClick={() => setMobileMenuOpen(true)}
            className="grid h-8 w-8 place-items-center rounded-[10px] text-[#6B7280] transition hover:bg-[#F8FAFC]">
            <Menu size={18} />
          </button>
        </div>
      </header>
      ...
      {/* ── Desktop Sidebar ── */}
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-56 flex-col border-r border-[#EEF2F7] bg-white lg:flex"
        style={{ boxShadow: '1px 0 0 #EEF2F7' }}>
        {/* Logo area */}
        <div className="flex h-[60px] shrink-0 items-center gap-3 border-b border-[#EEF2F7] px-5">
          <Brand />
        </div>
        <SidebarContent
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          <button type="button" className="absolute inset-0 bg-[#111827]/40 backdrop-blur-sm"
            onClick={() => setMobileMenuOpen(false)} aria-label="关闭" />
          <aside className="relative flex h-full w-56 flex-col border-r border-[#EEF2F7] bg-white">
            <div className="flex h-14 items-center justify-between border-b border-[#EEF2F7] px-4">
              <Brand />
              <button type="button" onClick={() => setMobileMenuOpen(false)}
                className="grid h-7 w-7 place-items-center rounded-[8px] text-[#9CA3AF] hover:bg-[#F8FAFC]">
                <X size={16} />
              </button>
            </div>
      ...
      <main className="min-w-0 flex-1 lg:ml-56">
        ...
        {/* Desktop top header */}
        <header className="sticky top-0 z-20 hidden h-[60px] items-center justify-between gap-4 border-b border-[#EEF2F7] bg-white px-8 lg:flex"
          style={{ boxShadow: '0 1px 0 #EEF2F7' }}>

          {/* Page title */}
          <div className="min-w-0">
            <h1 className="text-[16px] font-bold leading-tight tracking-[-0.01em] text-[#111827]">
              {pageMeta.title}
            </h1>
            <p className="mt-px text-[12px] text-[#9CA3AF]">{pageMeta.subtitle}</p>
          </div>
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <button type="button" onClick={() => void cloud.syncCloudNow()}
              className="grid h-8 w-8 place-items-center rounded-[8px] border border-[#EEF2F7] bg-white text-[#9CA3AF] transition hover:border-[#D1D5DB] hover:text-[#374151]">
            ...
            <button type="button" onClick={() => setPrivacyOpen(true)}
              className="inline-flex h-8 items-center gap-1.5 rounded-[8px] border border-[#EEF2F7] bg-white px-3 text-[12px] font-medium text-[#6B7280] transition hover:bg-[#F8FAFC]">
              <Eye size={13} />
              隐私设置
            </button>
            ...
            {/* Avatar */}
            <button type="button"
              className="grid h-8 w-8 place-items-center rounded-full bg-gradient-to-br from-[#2563EB] to-[#8B5CF6] text-[12px] font-bold text-white"
              aria-label="用户菜单">
              W
            </button>
          </div>
...
function Brand() {
  return (
    <div className="flex items-center gap-2.5">
      <div className="grid h-8 w-8 shrink-0 place-items-center rounded-[10px] bg-gradient-to-br from-[#2563EB] to-[#8B5CF6] text-white shadow-sm">
        <span className="text-[11px] font-black tracking-tight">HK</span>
      </div>
      <div className="min-w-0">
        <p className="text-[13px] font-bold leading-tight text-[#111827]">港新账本</p>
        <p className="text-[9px] font-medium tracking-[0.1em] text-[#9CA3AF]">PERSONAL INVESTMENT OS</p>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      <nav className="flex-1 overflow-y-auto px-3 py-3">
        {groups.map((group) => {
          ...
          return (
            <div key={group.label} className="mb-4">
              {/* Group label */}
              <p className="mb-1 px-2 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#9CA3AF]">
                {group.label}
              </p>
              {items.map((item) => (
        ...
        {/* AI 中心 */}
        <div className="mb-4">
          <p className="mb-1 px-2 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#9CA3AF]">AI中心</p>
          {aiItems.map(({ label, icon: Icon }) => (
            <button key={label} type="button" disabled
              className="flex w-full items-center gap-2.5 rounded-[8px] px-2 py-2 text-left text-[13px] text-[#9CA3AF] cursor-not-allowed">
              <Icon size={14} className="shrink-0 text-[#D1D5DB]" />
              <span className="flex-1">{label}</span>
              <span className="rounded-[4px] bg-[#F3F4F6] px-1.5 py-0.5 text-[9px] font-medium text-[#9CA3AF]">
                即将上线
              </span>
            </button>
          ))}
      ...
      <div className="shrink-0 border-t border-[#EEF2F7] px-4 py-4">
        <div className="flex items-center gap-1.5 text-[12px] font-medium text-[#374151]">
          <ShieldCheck size={12} className="shrink-0 text-[#22C55E]" />
          {cloud.cloudUser ? '本地 + 云端保护' : '本地数据保护'}
        </div>
        {cloud.cloudUser && (
          <p className="mt-1 truncate text-[11px] text-[#9CA3AF]">已登录 {cloud.cloudUser.email}，修改后自动同步。</p>
        )}
        {!cloud.cloudUser && (
          <p className="mt-1 text-[11px] text-[#9CA3AF]">数据保存在当前浏览器。</p>
        )}
        <p className="mt-1 text-[11px] text-[#9CA3AF]">最后同步: {syncLabel}</p>
        <p className="mt-3 text-[11px] font-medium text-[#D1D5DB]">v{APP_VERSION}</p>
      </div>
    </div>
  )
}
  ...
  return (
    ...
    >
      <Icon
        ...
        className={active ? 'text-[#2563EB]' : 'text-[#9CA3AF] group-hover:text-[#6B7280]'}
      />
      <span className="flex-1 leading-none">{label}</span>
      {!available && (
        <span className="rounded-[4px] bg-[#F3F4F6] px-1.5 py-0.5 text-[9px] text-[#9CA3AF]">soon</span>
      )}
    </button>
  )
}
...
function FilterPill({ icon: Icon, label }: { icon: typeof SlidersHorizontal; label: string }) {
  return (
    <button type="button"
      className="inline-flex h-8 items-center gap-1.5 rounded-[8px] border border-[#EEF2F7] bg-white px-3 text-[12px] font-medium text-[#374151] transition hover:bg-[#F8FAFC] hover:border-[#D1D5DB]">
      <Icon size={13} className="text-[#9CA3AF]" />
      {label}
      <ChevronDown size={12} className="text-[#D1D5DB]" />
    </button>
  )
}
  ...
  return (
    <button type="button" onClick={onClick}
      className={[
        ...
        hasProblem          ? 'border-[#FEF3C7] bg-[#FFFBEB] text-[#D97706]'
        : connected && pending ? 'border-[#DBEAFE] bg-[#EFF6FF] text-[#2563EB]'
        : connected            ? 'border-[#DCFCE7] bg-[#F0FDF4] text-[#16A34A]'
...
function InstallButton({ compact = false, onClick }: { compact?: boolean; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick}
      className="inline-flex h-8 items-center gap-1.5 rounded-[8px] bg-[#2563EB] px-3 text-[12px] font-semibold text-white transition hover:bg-[#1D4ED8]"
    ...
    <Modal open={open} title="安装到手机桌面" description="请先用手机浏览器打开部署后的网址。" onClose={onClose}>
      <div className="space-y-4 px-5 py-5">
        <div className="rounded-[10px] bg-[#F8FAFC] p-4">
          <p className="text-[13px] font-semibold text-[#111827]">iPhone / iPad</p>
          <p className="mt-2 flex items-start gap-2 text-[12px] leading-5 text-[#6B7280]">
            <Share size={14} className="mt-0.5 shrink-0 text-[#2563EB]" />
            使用 Safari 打开网址，点击底部"分享"，再选择"添加到主屏幕"。
          </p>
        </div>
        <div className="rounded-[10px] bg-[#F8FAFC] p-4">
          <p className="text-[13px] font-semibold text-[#111827]">Android</p>
          <p className="mt-2 text-[12px] leading-5 text-[#6B7280]">
            使用 Chrome 打开网址，点击浏览器菜单中的"安装应用"或"添加到主屏幕"。
          </p>
        </div>
        <p className="text-[11px] leading-5 text-[#D97706]">
