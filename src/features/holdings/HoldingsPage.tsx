    ...
    <>
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-950 sm:text-3xl">持仓管理</h1>
          <p className="mt-2 text-sm text-slate-500">管理长期持仓，并按券商抵押率计算可用于打新的融资额度。</p>
        </div>
        <button
          type="button"
          ...
          return (
            <article key={holding.id} className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-card">
              <div className="flex items-start justify-between gap-3">
                <div><h2 className="font-bold text-slate-900">{holding.stockName}（{holding.stockCode}）</h2><p className="mt-1 text-xs text-slate-400">{account ? formatAccountName(account) : '已删除账户'}</p></div>
