...
- 安装后支持独立窗口启动及基础离线访问
- Supabase 邮箱登录与多设备自动同步
- LocalStorage 离线副本及首次同步冲突保护
...
当前数据仍使用 LocalStorage 作为离线副本。未配置云同步时，可通过“数据管理”
导出 JSON，并在另一台设备导入；配置 Supabase 后可登录同一账号自动同步。

## Supabase 配置

1. 在 Supabase 创建项目。
2. 在 SQL Editor 执行
   `supabase/migrations/202606140001_create_user_data.sql`。
3. 从项目设置复制 Project URL 和 Publishable Key。
4. 按 `.env.example` 创建 `.env.local`。

迁移同时保存在 `prisma/migrations/`。Prisma schema 用于数据库结构管理，
浏览器运行时通过 Supabase Auth + PostgREST 访问，并由 RLS 隔离每个用户的数据。
